...
export async function POST(request: NextRequest) {
  try {
    ...
    // Parse additional context for personalized recommendations
    const context = additionalInfo?.toLowerCase() || "";
    
    // Determine report type and generate contextual analysis
    let analysisType = "blood";
    if (context.includes("urine") || context.includes("urinalysis")) {
      analysisType = "urine";
    } else if (context.includes("x-ray") || context.includes("xray")) {
      analysisType = "xray";
    } else if (context.includes("diabetes") || context.includes("sugar") || context.includes("glucose")) {
      analysisType = "diabetes";
    } else if (context.includes("thyroid") || context.includes("tsh")) {
      analysisType = "thyroid";
    } else if (context.includes("lipid") || context.includes("cholesterol")) {
      analysisType = "lipid";
    } else if (context.includes("liver") || context.includes("sgpt") || context.includes("sgot")) {
      analysisType = "liver";
    } else if (context.includes("kidney") || context.includes("creatinine") || context.includes("bun")) {
      analysisType = "kidney";
    } else if (context.includes("cbc") || context.includes("complete blood count")) {
      analysisType = "cbc";
    }

    // Generate comprehensive analysis based on report type
    const analysis = generateLabAnalysis(analysisType, context);
